...
function SellersList() {
    ...
    const [commissionModal, setCommissionModal] = useState({ open: false, seller: null });
    ...
    const openCommissionModal = (seller) => {
        ...
        setCommissionValue(seller.commissionPercentage ?? '5.00');
        setOpenActionsMenu(null);
    };
    ...
    const handleSaveCommission = async () => {
        ...
        try {
            await updateSellerCommission(commissionModal.seller.id, value);
            showNotification('success', `Comision actualizada a ${value}%`);
            setCommissionModal({ open: false, seller: null });
            loadSellers();
        } catch (error) {
            console.error('Error updating commission:', error);
            showNotification('error', error.response?.data?.message || 'Error al actualizar comision');
        } finally {
    ...
    return (
        <div className="bg-background-light dark:bg-background-dark text-slate-800 dark:text-slate-100 min-h-screen flex transition-colors duration-200">
            ...
            {commissionModal.open && (
                <div className="fixed inset-0 z-50 overflow-y-auto">
                    <div className="flex items-center justify-center min-h-screen px-4">
                        <div className="fixed inset-0 bg-black opacity-30" onClick={() => setCommissionModal({ open: false, seller: null })}></div>
                        <div className="relative bg-white dark:bg-surface-dark rounded-xl shadow-xl max-w-sm w-full p-6">
                            <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-1">Modificar Comision</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">
                                {commissionModal.seller?.fullName}
                            </p>
                            ...
                            <input
                                ...
                                className="w-full px-4 py-2.5 bg-background-light dark:bg-background-dark border border-border-light dark:border-border-dark rounded-lg text-sm text-slate-900 dark:text-white focus:border-primary focus:ring-1 focus:ring-primary transition-all"
                                placeholder="Ej: 5.00"
                            />
                            <div className="flex gap-3 justify-end mt-5">
                                <button
                                    onClick={() => setCommissionModal({ open: false, seller: null })}
                                    className="px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors"
                                >
                                    Cancelar
                                </button>
                                <button
                                    ...
                                    className="px-4 py-2 text-sm font-medium text-white bg-primary hover:bg-red-700 rounded-lg transition-colors disabled:opacity-50"
                                >
