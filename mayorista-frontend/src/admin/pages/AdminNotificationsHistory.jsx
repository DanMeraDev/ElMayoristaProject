...
const TYPE_LABELS = {
    ...
    SALE_PENDING_REMINDER: 'Recordatorio pendiente',
    SALE_PENDING_ADMIN_ALERT: 'Alerta de pago',
    ...
    CUSTOMER_FIADO_CREATED: 'Fiado a cliente',
};

const TYPE_COLORS = {
    SALE_CREATED: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400',
    SALE_PENDING_ADMIN_ALERT: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400',
    ...
    CUSTOMER_REGISTERED: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400',
    CUSTOMER_APPROVED: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400',
    FIADO_CREATED: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
    CUSTOMER_FIADO_CREATED: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
};
...
const getIconColor = (type, read) => {
    ...
    if (type === 'SALE_PENDING_ADMIN_ALERT' || type === 'SALE_REJECTED' || type === 'SALE_RETURNED') return 'text-red-600 dark:text-red-400';
    if (type === 'SALE_UNDER_REVIEW') return 'text-purple-600 dark:text-purple-400';
    if (type === 'SALE_APPROVED' || type === 'CUSTOMER_REGISTERED' || type === 'CUSTOMER_APPROVED') return 'text-green-600 dark:text-green-400';
    if (type === 'FIADO_CREATED' || type === 'CUSTOMER_FIADO_CREATED') return 'text-blue-600 dark:text-blue-400';
...
const getIconBg = (type, read) => {
    ...
    if (type === 'SALE_PENDING_ADMIN_ALERT' || type === 'SALE_REJECTED' || type === 'SALE_RETURNED') return 'bg-red-100 dark:bg-red-900/30';
    if (type === 'SALE_UNDER_REVIEW') return 'bg-purple-100 dark:bg-purple-900/30';
    if (type === 'SALE_APPROVED' || type === 'CUSTOMER_REGISTERED' || type === 'CUSTOMER_APPROVED') return 'bg-green-100 dark:bg-green-900/30';
    if (type === 'FIADO_CREATED' || type === 'CUSTOMER_FIADO_CREATED') return 'bg-blue-100 dark:bg-blue-900/30';
...
function AdminNotificationsHistory() {
    ...
    const handleNotificationClick = async (notification) => {
        ...
        if (type === 'FIADO_CREATED' || type === 'CUSTOMER_FIADO_CREATED') navigate('/admin/settings?tab=fiados');
        else if (type === 'CUSTOMER_REGISTERED') navigate('/admin/settings?tab=clientes');
        else if (type === 'SALE_CREATED') navigate('/admin/sales-history');
        else if (type === 'RANKING_ACHIEVEMENT') navigate('/admin/ranking');
